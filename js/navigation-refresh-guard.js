...
/* =========================================================
   PASAR UMKM - NAVIGATION / INITIAL RENDER GUARD v2
   - Static final menu links exist before late feature hydration.
   - Repeated Beranda tap refreshes fresh marketplace data without reload.
   - Deep-link profile reload still returns to the home shell.
   ========================================================= */
...
(() => {
  ...
  function ensureFinalNavigation() {
    ...
    if (!host.querySelector('[data-p7-launch-link]')) {
      host.appendChild(finalSideLink('/launch/', 'rocket-launch', 'Pusat Penjual', 'p7LaunchLink'));
    }
    if (!host.querySelector('[data-p7-legal-link]')) {
      host.appendChild(finalSideLink('/legal/', 'shield-check', 'Trust Center', 'p7LegalLink'));
    }
    if (!host.querySelector('[data-p8-purchases-link]')) {
      host.appendChild(finalSideLink('/purchases/', 'package', 'Pembelian Saya', 'p8PurchasesLink'));
    }
    return true;
  }
