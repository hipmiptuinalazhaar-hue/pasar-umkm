...
function metric(label, value, detail = "") {
  return `<div class="metric-item"><span class="metric-label">${label}</span><span class="metric-value">${value}</span>${detail ? `<span class="metric-detail">${detail}</span>` : ""}</div>`;
}

function queue(label, detail, count, href) {
  return `<li class="queue-item"><div class="queue-name"><strong>${label}</strong><span>${detail}</span></div><div><span class="queue-count">${number.format(count)}</span><a class="row-action" href="${href}">Buka</a></div></li>`;
}
...
export async function renderOverview({ host, access, signal }) {
  ...
  const sessionSecurity = access.session?.mfa_verified ? { label: "MFA verified", className: "status-success" } : { label: "Session active", className: "status-info" };

  host.innerHTML = `
    <header class="view-header"><div><p class="eyebrow">Operational Overview</p><h1 class="view-title">Control Center</h1><p class="view-description">Ringkasan real-time dari data marketplace yang tersedia. Tidak ada statistik placeholder atau estimasi buatan.</p></div><div class="view-actions"><span class="status ${sessionSecurity.className}">${sessionSecurity.label}</span></div></header>
    ...
    <section class="section-block"><div class="section-head"><div><h2 class="section-title">Operational queues</h2><p class="section-copy">Antrian nyata yang membutuhkan perhatian administrator.</p></div></div><ul class="queue-list">
    ...
    <section class="section-block"><div class="section-head"><div><h2 class="section-title">Commerce & trust</h2><p class="section-copy">Kondisi transaksi dan sinyal kualitas yang berasal dari database produksi.</p></div></div><ul class="access-list">
    ...
    <section class="security-note" aria-label="Security context"><div><strong>${access.roles.map(role => role.name).join(", ")}</strong><p>${access.permissions.length} permission aktif, ${sensitiveCount} di antaranya ditandai sensitive. Permission sensitif membutuhkan step-up MFA yang masih fresh.</p></div></section>`;
}
