...
export async function renderOperations({ host, permissionSet, signal, confirmAction, refresh }) {
  ...
  host.innerHTML = `
    ...
    <section class="section-block"><div class="section-head"><div><h2 class="section-title">Verifikasi toko</h2><p class="section-copy">Pengajuan identitas usaha yang menunggu pemeriksaan administrator.</p></div><span class="queue-count">${number.format(safeNumber(data.pending_verification_cases))}</span></div><ul class="access-list">${verifications.length ? verifications.map(item => verificationRow(item, canVerify)).join("") : empty("pengajuan verifikasi")}</ul></section>
    <section class="section-block"><div class="section-head"><div><h2 class="section-title">Laporan moderasi</h2><p class="section-copy">Laporan pengguna adalah sinyal untuk ditinjau, bukan keputusan otomatis terhadap akun atau konten.</p></div><span class="queue-count">${number.format(safeNumber(data.open_reports))}</span></div><ul class="access-list">${reports.length ? reports.map(item => reportRow(item, canResolveReports)).join("") : empty("laporan terbuka")}</ul></section>
    <section class="section-block"><div class="section-head"><div><h2 class="section-title">Sengketa pesanan</h2><p class="section-copy">Kasus pembeli dan penjual untuk mediasi operasional. Tidak ada settlement atau refund otomatis.</p></div><span class="queue-count">${number.format(safeNumber(data.open_disputes))}</span></div><ul class="access-list">${disputes.length ? disputes.map(item => disputeRow(item, canResolveDisputes)).join("") : empty("sengketa aktif")}</ul></section>
