...
export async function renderSupport(context){
  ...
  function bindDetail(){
    ...
    detailHost.querySelector('[data-cs-password-reset]')?.addEventListener('click',async event=>{
      if(!currentId)return;
      const confirmed=window.confirm('Pastikan identitas pengguna sudah diverifikasi secara manual. Reset kata sandi sekarang? Semua sesi lama pengguna akan dicabut.');
      if(!confirmed)return;
      const button=event.currentTarget;button.disabled=true;button.textContent='Mereset…';
      try{
        const result=await adminApi.supportPasswordReset(currentId);
        const password=String(result.temporary_password||'');
        if(!password)throw new Error('Kata sandi sementara tidak diterima dari server.');
        window.prompt('RESET BERHASIL. Salin kata sandi sementara ini dan berikan HANYA kepada pengguna yang identitasnya sudah diverifikasi. Kata sandi ini tidak dapat ditampilkan ulang:',password);
        await openTicket(currentId,{silent:true});
      }catch(error){alert(error.message||'Reset kata sandi gagal.');button.disabled=false;button.textContent='Reset kata sandi';}
    });
