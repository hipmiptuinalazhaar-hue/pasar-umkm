"use strict";const CONFIG=Object.freeze({APP_NAME:"Pasar UMKM",CITY:"Lubuklinggau",ORGANIZATION:"HIPMI PT UIN Al Azhaar Lubuklinggau",INITIATOR:"Capryan Agusto",DEMO_MODE:!1,API_BASE_URL:"",STORAGE_KEY:"pasar-umkm-ui-v7",INTRO_KEY:"pasar-umkm-intro-v7",SPLASH_DURATION:950,SPLASH_EXIT_DURATION:320,TOAST_DURATION:2200,SEARCH_MIN_LENGTH:2}),ASSETS=Object.freeze({logo:"assets/logo.webp"});let CATEGORIES=[];const DATA={stories:[],posts:[],stores:[],notifications:[],messages:[],orders:[]},STATE={user:null,currentStore:null,activeNav:"home",activeCategory:null,searchQuery:"",likedPosts:new Set,savedPosts:new Set,cart:[],accountProducts:[],menuOpen:!1,searchOpen:!1,activeSheet:null,loading:!1},CATALOG_PAGE_LIMIT=24,CATALOG_PAGINATION={products:{nextCursor:null,hasNext:!1,loading:!1},stores:{nextCursor:null,hasNext:!1,loading:!1}};let catalogIntersectionObserver=null;function applyCatalogPagination(target,pagination){const nextCursor=String(pagination?.next_cursor||"").trim();target.nextCursor=nextCursor||null,target.hasNext=!!(pagination?.has_next&&nextCursor)}const DOM={};document.addEventListener("DOMContentLoaded",initializeApp);async function initializeApp(){cacheDOM(),restoreLocalState(),bindEvents(),document.querySelector("[data-v10-lazy=reels]")&&navigate("reels"),setupSplash(),setLoading(!0);try{await loadInitialData()}catch(error){console.error("[Pasar UMKM] Bootstrap error:",error),showToast("Data belum dapat dimuat.")}finally{setLoading(!1)}renderApplication(),handleScroll()}function cacheDOM(){DOM.splash=document.getElementById("splashIntro"),DOM.header=document.querySelector(".app-header")||document.getElementById("header"),DOM.storiesSection=document.getElementById("storiesSection"),DOM.stories=document.getElementById("stories"),DOM.homeDiscovery=document.getElementById("homeDiscovery"),DOM.quickCategories=document.getElementById("quickCategories"),DOM.feed=document.getElementById("feed"),DOM.menuButton=document.getElementById("menuButton"),DOM.headerSearchButton=document.getElementById("headerSearchButton"),DOM.notificationButton=document.getElementById("notificationButton"),DOM.messageButton=document.getElementById("messageButton"),DOM.sideMenu=document.getElementById("sideMenu"),DOM.closeMenuButton=document.getElementById("closeMenuButton"),DOM.sideMenuContent=document.getElementById("sideMenuContent"),DOM.sideAccountGuest=document.getElementById("sideAccountGuest"),DOM.sideAccountUser=document.getElementById("sideAccountUser"),DOM.sideAccountUserName=document.getElementById("sideAccountUserName"),DOM.sideAccountUserRole=document.getElementById("sideAccountUserRole"),DOM.searchOverlay=document.getElementById("searchOverlay"),DOM.closeSearchButton=document.getElementById("closeSearchButton"),DOM.searchInput=document.getElementById("searchInput"),DOM.searchClearButton=document.getElementById("searchClearButton"),DOM.searchResults=document.getElementById("searchResults"),DOM.navigation=document.getElementById("appNavigation"),DOM.sheetOverlay=document.getElementById("sheetOverlay"),DOM.bottomSheet=document.getElementById("bottomSheet"),DOM.sheetContent=document.getElementById("sheetContent"),DOM.toast=document.getElementById("toast"),DOM.loading=document.getElementById("appLoading")}function fetchWithTimeout(input,init={},timeoutMs=1e4){const controller=new AbortController,timer=window.setTimeout(()=>controller.abort(),timeoutMs);return fetch(input,{...init,signal:controller.signal}).finally(()=>window.clearTimeout(timer))}async function resilientFetch(input,init={},timeoutMs=1e4,retries=1){let lastError=null;for(let attempt=0;attempt<=retries;attempt+=1)try{return await fetchWithTimeout(input,init,timeoutMs)}catch(error){if(lastError=error,attempt>=retries)break;await new Promise(resolve=>window.setTimeout(resolve,300))}throw lastError||new Error("Permintaan jaringan gagal.")}async function loadInitialData(){if(await restoreAuthSession(),STATE.currentStore=null,STATE.user?.role==="seller"||STATE.user?.role==="admin")loadCurrentAccountStore().then(store=>{STATE.currentStore=store}).catch(error=>{console.error("[Pasar UMKM] Current store background load error:",error),STATE.currentStore=null});await Promise.allSettled([loadCategories(),loadStores()]);const[productsResult,postsResult]=await Promise.allSettled([resilientFetch(`/api/products?limit=${CATALOG_PAGE_LIMIT}`,{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"},1e4,1),resilientFetch("/api/posts",{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"},1e4,1)]),feedItems=[];if(productsResult.status==="fulfilled"){const response=productsResult.value;if(response.ok){const data=await response.json().catch(()=>({}));data.ok===!0&&Array.isArray(data.products)&&(applyCatalogPagination(CATALOG_PAGINATION.products,data.pagination),feedItems.push(...data.products.map(createPublicProductFeedPost)))}else console.error("[Pasar UMKM] Products bootstrap HTTP error:",response.status)}else console.error("[Pasar UMKM] Products bootstrap network error:",productsResult.reason);if(postsResult.status==="fulfilled"){const response=postsResult.value;if(response.ok){const data=await response.json().catch(()=>({}));data.ok===!0&&Array.isArray(data.posts)&&feedItems.push(...data.posts.map(post=>{const location=[post.store_district,post.store_city].filter(Boolean).join(", ")||CONFIG.CITY;return{id:`post-${post.id}`,backendId:post.id,store:{id:post.store_id,name:post.store_name||"UMKM Lokal",avatar:post.store_logo_url||ASSETS.logo,location,verified:post.store_verification_status==="verified"},location,caption:post.caption||"",createdAt:post.created_at,media:{type:"image",src:post.image_url,alt:post.caption||`Postingan ${post.store_name||"UMKM"}`},likesCount:0,commentsCount:Number(post.comments_count||0)}}))}else console.error("[Pasar UMKM] Posts bootstrap HTTP error:",response.status)}else console.error("[Pasar UMKM] Posts bootstrap network error:",postsResult.reason);if(feedItems.sort((x,y)=>new Date(y.createdAt||0).getTime()-new Date(x.createdAt||0).getTime()),DATA.posts=feedItems,!CONFIG.API_BASE_URL)return;try{const bootstrap=await apiRequest("/api/bootstrap");bootstrap&&(DATA.stories=ensureArray(bootstrap.stories),Array.isArray(bootstrap.posts)&&bootstrap.posts.length&&(DATA.posts=ensureArray(bootstrap.posts)),DATA.notifications=ensureArray(bootstrap.notifications),DATA.messages=ensureArray(bootstrap.messages),DATA.orders=ensureArray(bootstrap.orders),bootstrap.user&&(STATE.user=bootstrap.user),Array.isArray(bootstrap.cart)&&(STATE.cart=bootstrap.cart))}catch(error){console.error("[Pasar UMKM] Optional bootstrap error:",error)}}async function loadCategories(){try{const response=await resilientFetch("/api/categories",{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"});if(!response.ok)throw new Error(`Categories request failed: ${response.status}`);const data=await response.json();if(data.ok!==!0||!Array.isArray(data.categories))throw new Error("Format data kategori tidak valid.");CATEGORIES=data.categories.map(category=>({id:String(category.id||""),slug:String(category.slug||""),name:String(category.name||""),icon:String(category.icon||"tag"),home:!!category.is_home,sortOrder:Number(category.sort_order)||0})).filter(category=>category.id&&category.name).sort((a,b)=>a.sortOrder-b.sortOrder)}catch(error){console.error("[Pasar UMKM] Categories load error:",error),CATEGORIES=[],showToast("Kategori belum dapat dimuat.")}}async function loadStores({append=!1}={}){const state=CATALOG_PAGINATION.stores;if(append&&(!state.hasNext||!state.nextCursor||state.loading))return[];state.loading=!0;try{const params=new URLSearchParams({limit:String(CATALOG_PAGE_LIMIT)});append&&state.nextCursor&&params.set("cursor",state.nextCursor);const response=await resilientFetch(`/api/stores?${params.toString()}`,{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"}),data=await response.json().catch(()=>({}));if(!response.ok||data.ok!==!0||!Array.isArray(data.stores))throw new Error(data.error||`Stores request failed: ${response.status}`);const normalized=data.stores.map(normalizePublicStore).filter(store=>store.id&&store.name);if(append){const seen=new Set(DATA.stores.map(store=>String(store.id)));DATA.stores.push(...normalized.filter(store=>!seen.has(String(store.id))))}else DATA.stores=normalized;return applyCatalogPagination(state,data.pagination),normalized}catch(error){return console.error("[Pasar UMKM] Stores load error:",error),append?showToast(error.message||"UMKM berikutnya belum dapat dimuat."):(DATA.stores=[],state.nextCursor=null,state.hasNext=!1,showToast("Daftar UMKM belum dapat dimuat.")),[]}finally{state.loading=!1}}function normalizePublicStore(store){return{id:String(store.id||""),categoryId:String(store.category_id||""),category:String(store.category_name||""),name:String(store.name||""),slug:String(store.slug||""),description:String(store.description||""),logo:String(store.logo_url||""),cover:String(store.cover_url||""),phone:String(store.phone||""),whatsapp:String(store.whatsapp||""),address:String(store.address||""),district:String(store.district||""),city:String(store.city||""),province:String(store.province||""),verificationStatus:String(store.verification_status||"pending"),verifiedAt:store.verified_at||null,productCount:Number(store.product_count||0),createdAt:store.created_at||null}}function createPublicProductFeedPost(product){return{id:`product-${product.id}`,store:{id:product.store_id,name:product.store_name||"UMKM Lokal",avatar:product.store_logo_url||ASSETS.logo,location:CONFIG.CITY,verified:product.store_verification_status==="verified"},caption:product.description||"",createdAt:product.created_at,commentsCount:Number(product.comments_count||0),product:{id:product.id,name:product.name,image:product.image_url||ASSETS.logo,category:product.category_name||"",categoryId:product.category_id||"",price:Number(product.price||0),stock:Number(product.stock||0),unit:product.unit||""}}}async function loadMoreProducts(){const state=CATALOG_PAGINATION.products;if(!state.hasNext||!state.nextCursor||state.loading)return;state.loading=!0;const button=document.querySelector("[data-catalog-sentinel]")?.querySelector("button");button&&(button.disabled=!0);try{const params=new URLSearchParams({limit:String(CATALOG_PAGE_LIMIT),cursor:state.nextCursor}),response=await fetch(`/api/products?${params.toString()}`,{method:"GET",credentials:"include",headers:{Accept:"application/json"},cache:"no-store"}),data=await response.json().catch(()=>({}));if(!response.ok||data.ok!==!0||!Array.isArray(data.products))throw new Error(data.error||`Products request failed: ${response.status}`);const existing=new Set(DATA.posts.map(post=>String(post.id||"")));for(const product of data.products){const post=createPublicProductFeedPost(product);existing.has(String(post.id))||(DATA.posts.push(post),existing.add(String(post.id)))}if(applyCatalogPagination(state,data.pagination),STATE.activeCategory){const category=CATEGORIES.find(item=>item.id===STATE.activeCategory)||null;category&&renderFeed(DATA.posts.filter(post=>normalizeText(post.product?.category)===normalizeText(category.name)||normalizeText(post.product?.categoryId)===normalizeText(category.id)),category)}else STATE.activeNav==="home"&&renderFeed()}catch(error){console.error("[Pasar UMKM] Product pagination error:",error),showToast(error.message||"Produk berikutnya belum dapat dimuat.")}finally{state.loading=!1,button?.isConnected&&(button.disabled=!1)}}async function loadMoreStores(){const state=CATALOG_PAGINATION.stores;if(!state.hasNext||!state.nextCursor||state.loading)return;const previousScroll=DOM.sheetContent?.scrollTop||0;await loadStores({append:!0}),openStores(),requestAnimationFrame(()=>{DOM.sheetContent&&(DOM.sheetContent.scrollTop=previousScroll)})}function createCatalogLoadMoreTemplate(category=null){return CATALOG_PAGINATION.products.hasNext?`
  ...
  `}function openAllCategories(){const categories=CATEGORIES.map(category=>`
    ...
    ${createCatalogLoadMoreTemplate(category)}
  `,scheduleCatalogPaginationObserver(category)}function createEmptyFeedTemplate(category=null){return category?`
