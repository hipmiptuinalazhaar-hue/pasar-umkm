...
const assertions = [
  ...
  ['V10 public cache is bounded', v10a.includes('PUBLIC_CACHE_MAX_ENTRIES = 64') && v10a.includes('responseCache.size >= PUBLIC_CACHE_MAX_ENTRIES') && v10a.includes('responseCache.keys().next().value')],
  ['V10 stale public cache entries are removed', v10a.includes('cached?.expiresAt > now') && v10a.includes('else if (cached) responseCache.delete(key)')],
  ['V10 public network diagnostics increment', v10a.includes('warmRequests += 1') && v10a.includes('warm_requests: warmRequests')],
