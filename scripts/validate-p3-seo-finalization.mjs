...
const checks = [
  ...
  ['store LocalBusiness schema', seo.includes('"@type": "LocalBusiness"') && seo.includes('logo: store.logo_url || LOGO_URL')],
  ['product seller Organization logo', seo.includes('logo: product.store_logo_url || LOGO_URL')],
  ['short public descriptions are expanded', seo.includes('function descriptiveMeta(') && seo.includes('if (base.length >= 70) return base')],
  ['store crawl-quality detail section', seo.includes('Informasi tentang ${escapeHtml(store.name)}')],
  ['product crawl-quality detail section', seo.includes('Informasi produk ${escapeHtml(product.name)}')],
