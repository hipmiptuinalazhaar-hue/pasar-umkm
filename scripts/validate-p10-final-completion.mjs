...
const assertions = [
  ...
  ['growth integrity gate is canonical', validate.includes('test:growth-integrity')],
  ...
  ['growth integrity validator exists', exists('scripts/validate-growth-integrity-v2.mjs')],
  ...
  ['P10 documentation defines completion target', finalDoc.includes('100% engineering completion')],
  ['P10 documentation declares no-Actions release authority', finalDoc.includes('GitHub Actions are not an active release dependency')],
  ['P10 documentation refuses to equate unavailable observation with PASS', finalDoc.includes('Not observable from the current tool environment')],
  ['P10 documentation records unprotected main governance state', finalDoc.includes('`main` had no active protection/ruleset')],
  ['P10 documentation records manual-only Neon recovery anchor', finalDoc.includes('Automatic snapshot scheduling is not currently configured')],
  ['P10 documentation requires exact Cloudflare SHA before production probes', finalDoc.includes('release:postdeploy') && finalDoc.includes('exact local HEAD SHA')],
