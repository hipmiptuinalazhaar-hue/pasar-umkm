...
async function audit(authz,request,{action,resourceId=null,outcome="success",reasonCode=null,metadata={}}){
  ...
  await authz.sql`
    ...
    VALUES(${authz.session.id},${authz.session.name},${authz.session.email},${action},'promotion',${resourceId},${outcome},${reasonCode},${requestIdentifier(request)},${ipHash},${userAgentHash},CAST(${JSON.stringify(metadata)} AS jsonb))
  `;
}
function ratio(numerator,denominator){const a=Number(numerator||0),b=Number(denominator||0);return b>0?Number(((a/b)*100).toFixed(2)):0;}

function normalizeDestinationUrl(value,{external=false}={}){
  const raw=String(value||"").trim().slice(0,800);
  if(!raw)return null;
  if(!external&&raw.startsWith("/")&&!raw.startsWith("//"))return raw;
  let parsed;
...
async function metrics(request,env){
  const authz=await requireAdminPermission(request,env,"growth.view");if(!authz.ok)return authz.response;
  const rows=await authz.sql`
    WITH base AS (
      SELECT event_name,COALESCE(user_id::text,anonymous_key_hash,id::text) AS actor_key,created_at
      FROM growth_events WHERE created_at>=NOW()-INTERVAL '30 days'
    ),counts AS (
      SELECT
...
async function listPromotions(request,env,url){
  const authz=await requireAdminPermission(request,env,"promotions.view");if(!authz.ok)return authz.response;
  const status=PROMOTION_STATUSES.has(url.searchParams.get("status"))?url.searchParams.get("status"):"all";
  const rows=await authz.sql`
    SELECT mp.id,mp.placement,mp.subject_type,mp.subject_id,mp.store_id,mp.headline,mp.sponsor_label,mp.destination_url,mp.status,mp.starts_at,mp.ends_at,mp.created_at,mp.updated_at,s.name AS store_name
    FROM marketplace_promotions mp LEFT JOIN stores s ON s.id=mp.store_id
    WHERE (${status}='all' OR mp.status=${status}) ORDER BY mp.created_at DESC,mp.id DESC LIMIT 100
  `;
...
async function subjectExists(sql,type,id){
  ...
  if(type==="product")return(await sql`SELECT EXISTS(SELECT 1 FROM products WHERE id=${id}::uuid AND is_active=TRUE) AS ok`)[0]?.ok===true;
  if(type==="store")return(await sql`SELECT EXISTS(SELECT 1 FROM stores WHERE id=${id}::uuid AND is_active=TRUE) AS ok`)[0]?.ok===true;
  if(type==="post")return(await sql`SELECT EXISTS(SELECT 1 FROM posts WHERE id=${id}::uuid AND is_active=TRUE) AS ok`)[0]?.ok===true;
  return false;
}
function parseTimestamp(value){if(!value)return null;const date=new Date(value);return Number.isFinite(date.getTime())?date.toISOString():null;}

async function createPromotion(request,env){
  if(!sameOrigin(request))return json({ok:false,code:"ORIGIN_REJECTED"},403);
  const authz=await requireAdminPermission(request,env,"promotions.manage");if(!authz.ok)return authz.response;
  const parsed=await smallJson(request);if(parsed.error)return parsed.error;
  const placement=String(parsed.body.placement||"").trim();
  const subjectType=String(parsed.body.subject_type||"").trim();
  const rawSubjectId=String(parsed.body.subject_id||"").trim()||null;
  const subjectId=subjectType==="external"?null:rawSubjectId;
  ...
  const destinationUrl=normalizeDestinationUrl(parsed.body.destination_url,{external:subjectType==="external"});
  const status=PROMOTION_STATUSES.has(parsed.body.status)?parsed.body.status:"draft";
  const startsAt=parseTimestamp(parsed.body.starts_at),endsAt=parseTimestamp(parsed.body.ends_at);
  ...
  if(subjectType==="external"&&!destinationUrl)return json({ok:false,code:"HTTPS_DESTINATION_REQUIRED"},400);
  if(parsed.body.destination_url&& !destinationUrl)return json({ok:false,code:"INVALID_DESTINATION_URL"},400);
  if(storeId&&!UUID_PATTERN.test(storeId))return json({ok:false,code:"INVALID_STORE_ID"},400);
  if(storeId){const exists=(await authz.sql`SELECT EXISTS(SELECT 1 FROM stores WHERE id=${storeId}::uuid) AS ok`)[0]?.ok===true;if(!exists)return json({ok:false,code:"STORE_NOT_FOUND"},404);}
  if(startsAt&&endsAt&&new Date(endsAt)<=new Date(startsAt))return json({ok:false,code:"INVALID_SCHEDULE"},400);

  const rows=await authz.sql`
    INSERT INTO marketplace_promotions(placement,subject_type,subject_id,store_id,headline,sponsor_label,destination_url,status,starts_at,ends_at,created_by_admin_id)
    VALUES(${placement},${subjectType},${subjectId}::uuid,${storeId}::uuid,${headline},${sponsorLabel},${destinationUrl},${status},${startsAt},${endsAt},${authz.session.id}) RETURNING *
  `;
...
async function changePromotionStatus(request,env,id){
  ...
  const authz=await requireAdminPermission(request,env,"promotions.manage");if(!authz.ok)return authz.response;
  const parsed=await smallJson(request);if(parsed.error)return parsed.error;
  const status=String(parsed.body.status||"").trim(),reason=String(parsed.body.reason||"").trim().slice(0,300);
  ...
  const rows=await authz.sql`UPDATE marketplace_promotions SET status=${status},updated_at=NOW() WHERE id=${id}::uuid RETURNING *`;
