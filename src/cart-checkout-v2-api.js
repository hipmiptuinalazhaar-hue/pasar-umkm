...
const text = (value, max) => { const valueText = String(value ?? "").trim(); return valueText ? valueText.slice(0, max) : null; };
const required = (value, min, max) => { const valueText = String(value ?? "").trim().slice(0, max); return valueText.length >= min ? valueText : null; };
const uuid = value => { const id = String(value || "").trim().toLowerCase(); return UUID.test(id) ? id : null; };
...
function coord(value, kind) {
  ...
  const valid = Number.isFinite(number) && (kind === "lat" ? number >= -90 && number <= 90 : number >= -180 && number <= 180);
...
function addressInput(body, user = {}) {
  ...
  const latitude = coord(body?.latitude, "lat");
  const longitude = coord(body?.longitude, "lng");
  ...
  if (Number.isNaN(latitude) || Number.isNaN(longitude) || (latitude === null) !== (longitude === null)) {
    throw Object.assign(new Error("Koordinat lokasi tidak valid."), { status: 400 });
  }
  if (Number.isNaN(accuracyM)) throw Object.assign(new Error("Akurasi lokasi tidak valid."), { status: 400 });
  return {
    label: text(body?.label, 60) || "Rumah",
    recipient_name: recipient,
    phone,
    address_text: address,
    district: text(body?.district, 100),
    city: text(body?.city, 100) || "Lubuklinggau",
    province: text(body?.province, 100) || "Sumatera Selatan",
    postal_code: text(body?.postal_code, 20),
...
async function addressBook(request, env, url) {
  ...
  const id = match ? uuid(match[1]) : null;
  try {
    if (url.pathname === "/api/commerce/address-book" && request.method === "GET") {
      const addresses = await sql`
        SELECT id,label,recipient_name,phone,address_text,district,city,province,postal_code,landmark,
               latitude,longitude,accuracy_m,is_default,created_at,updated_at
        FROM user_addresses
    ...
    if (url.pathname === "/api/commerce/address-book" && request.method === "POST") {
      ...
      const input = addressInput(body, auth.user);
      const count = await sql`SELECT COUNT(*)::int AS count FROM user_addresses WHERE user_id=${auth.user.id}`;
      const makeDefault = input.is_default || Number(count[0]?.count || 0) === 0;
    ...
    if (id && request.method === "PATCH") {
      const owns = await sql`SELECT id FROM user_addresses WHERE id=${id}::uuid AND user_id=${auth.user.id} LIMIT 1`;
      ...
      const rows = await sql`
        ...
        WHERE id=${id}::uuid AND user_id=${auth.user.id}
        RETURNING *
      `;
    ...
    if (id && request.method === "DELETE") {
      const rows = await sql`DELETE FROM user_addresses WHERE id=${id}::uuid AND user_id=${auth.user.id} RETURNING id,is_default`;
      if (!rows[0]) return error("Alamat tidak ditemukan.", 404, "ADDRESS_NOT_FOUND");
...
function orderNumber() {
  return `PUMKM-${Date.now().toString().slice(-10)}-${crypto.randomUUID().replace(/-/g, "").slice(0, 6).toUpperCase()}`;
}

function txError(message, status = 409, code = "CHECKOUT_CONFLICT") {
...
async function selectiveCheckout(request, env, url) {
  ...
  const raw = Array.isArray(body.selected_product_ids) ? body.selected_product_ids : [];
  const rawUnique = new Set(raw.map(value => String(value || "").trim().toLowerCase()));
  const selected = [...new Set(raw.map(uuid).filter(Boolean))].slice(0, MAX_SELECTED);
  ...
  const customerName = required(body.customer_name ?? auth.user.name, 2, 120);
  const customerPhone = required(body.customer_phone ?? auth.user.phone, 5, 30);
  const typedAddress = text(body.delivery_address, 1200);
  const landmark = text(body.delivery_landmark, 240);
  const latitude = coord(body.delivery_latitude, "lat");
  const longitude = coord(body.delivery_longitude, "lng");
  ...
  try {
    ...
    if (!cartId) throw txError("Keranjang masih kosong.", 409, "CART_EMPTY");
    ...
    if (itemResult.rows.length !== selected.length) throw txError("Ada produk pilihan yang sudah berubah atau tidak lagi ada di keranjang.", 409, "SELECTION_STALE");
    ...
    const products = new Map(productResult.rows.map(row => [String(row.id), row]));
    const groups = new Map();
    for (const cartItem of itemResult.rows) {
      const product = products.get(String(cartItem.product_id));
      if (!product || !product.is_active || !product.store_active) throw txError("Ada produk pilihan yang tidak lagi tersedia.", 409, "PRODUCT_UNAVAILABLE");
      const quantity = Number(cartItem.quantity || 0);
      const stock = Number(product.stock || 0);
      const price = Number(product.price || 0);
      if (!Number.isInteger(quantity) || quantity < 1) throw txError("Jumlah produk di keranjang tidak valid.");
      if (quantity > stock) throw txError(`Stok ${product.name || "produk"} tidak mencukupi.`, 409, "STOCK_CHANGED");
      const storeId = String(product.store_id);
      if (!groups.has(storeId)) groups.set(storeId, []);
      groups.get(storeId).push({ product_id: product.id, name: product.name, price, quantity, owner_id: product.owner_id });
    }
    ...
    if (prefResult.rows.length !== storeIds.length) throw txError("Pilihan pengiriman atau pembayaran berubah. Periksa checkout kembali.", 409, "PREFERENCE_REQUIRED");
    const prefs = new Map(prefResult.rows.map(row => [String(row.store_id), row]));

    const orders = [];
    for (const [storeId, items] of groups) {
      const pref = prefs.get(storeId);
      const subtotal = items.reduce((sum, item) => sum + item.price * item.quantity, 0);
      const settingsResult = await client.query("SELECT * FROM store_commerce_settings WHERE store_id=$1::uuid FOR SHARE", [storeId]);
      const settings = normalizeStoreCommerceSettings({ store_id: storeId, ...(settingsResult.rows[0] || {}) });
      const commerce = resolveCheckoutCommerce(settings, pref, subtotal);

      if (commerce.fulfillment_method !== pref.fulfillment_method || commerce.payment_method !== pref.payment_method) {
        throw txError("Metode pengiriman atau pembayaran baru saja berubah. Periksa checkout kembali.", 409, "COMMERCE_OPTIONS_CHANGED");
      }
      if (commerce.requires_address && !effectiveAddress) {
        throw txError("Pesanan yang diantar membutuhkan alamat atau titik lokasi.", 400, "ADDRESS_REQUIRED");
      }

      const delivery = commerce.requires_address;
      const deliveryFee = Number(commerce.delivery_fee || 0);
      const total = subtotal + deliveryFee;
      const orderNote = text(notesByStore[storeId], 500);
      const estimateMinutes = Math.max(0, Number(commerce.estimated_max_minutes || 0));

      const inserted = await client.query(
        `INSERT INTO orders(
          order_number,buyer_id,store_id,status,subtotal,delivery_fee,total,
          customer_name,customer_phone,delivery_address,notes,
          fulfillment_method,fulfillment_status,payment_method,payment_instructions,
          delivery_district,delivery_city,estimated_fulfillment_at,
          delivery_latitude,delivery_longitude,delivery_accuracy_m,delivery_landmark
        ) VALUES(
          $1,$2::uuid,$3::uuid,'pending',$4,$5,$6,$7,$8,$9,$10,
          $11,'awaiting_confirmation',$12,$13,$14,$15,
          CASE WHEN $16::int>0 THEN NOW()+($16::int*INTERVAL '1 minute') ELSE NULL END,
          $17,$18,$19,$20
        ) RETURNING *`,
        [
          orderNumber(), auth.user.id, storeId, subtotal, deliveryFee, total,
          customerName, customerPhone, delivery ? effectiveAddress : "Ambil di toko", orderNote,
          commerce.fulfillment_method, commerce.payment_method, commerce.payment_instructions,
          pref.delivery_district || null, pref.delivery_city || "Lubuklinggau", estimateMinutes,
          delivery ? latitude : null, delivery ? longitude : null, delivery ? accuracyM : null, delivery ? landmark : null
        ]
      );
      const order = inserted.rows[0];
      if (!order) throw txError("Pesanan gagal dibuat.", 500, "ORDER_CREATE_FAILED");
      ...
      for (const item of items) {
        ...
        if (!stockResult.rows[0]) throw txError(`Stok ${item.name || "produk"} berubah. Periksa checkout kembali.`, 409, "STOCK_CHANGED");
        await client.query(
    ...
    await client.query("DELETE FROM cart_items WHERE cart_id=$1::uuid AND product_id=ANY($2::uuid[])", [cartId, selected]);
    await client.query("COMMIT");
    active = false;
    return json({ ok: true, message: "Pesanan berhasil dibuat.", count: orders.length, orders, checked_out_product_ids: selected }, 201);
  } catch (cause) {
    if (active) {
      try { await client.query("ROLLBACK"); }
      catch (rollbackError) { console.error("Cart Checkout V2 rollback failed:", rollbackError); }
    }
    ...
    catch (closeError) { console.error("Cart Checkout V2 client close failed:", closeError); }
  }
}

export async function handleCartCheckoutV2Api(request, env) {
  const url = new URL(request.url);
  const address = await addressBook(request, env, url);
