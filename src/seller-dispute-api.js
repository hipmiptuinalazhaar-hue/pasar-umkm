...
async function sellerDisputes(request, env) {
  ...
  `;
  if (!stores.length) return json({ ok: true, stores: [], disputes: [] });
  const storeIds = stores.map(store => store.id);

  const rows = await sql`
    SELECT d.id,d.order_id,d.store_id,d.reason_code,d.description,d.status,
      ...
      u.name AS buyer_name,s.name AS store_name
    FROM order_disputes d
    JOIN orders o ON o.id=d.order_id
    JOIN users u ON u.id=d.buyer_id
    JOIN stores s ON s.id=d.store_id
    WHERE d.store_id=ANY(${storeIds}::uuid[])
    ...
    LIMIT 250
  `;
  return json({ ok: true, stores, disputes: rows });
}
